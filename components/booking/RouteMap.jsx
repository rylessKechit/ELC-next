...
const RouteMap = ({ pickupAddress, dropoffAddress, pickupPlaceId, dropoffPlaceId, polyline }) => {
      ...
      if (pickupPlaceId && dropoffPlaceId) {
        ...
        try {
          ...
          const directionsRenderer = new window.google.maps.DirectionsRenderer({
            ...
            polylineOptions: {
              strokeColor: '#d4af37', // Couleur primaire d'ELC
              strokeWeight: 5,
              strokeOpacity: 0.8
            }
          });
          ...
          try {
            addCustomMarker(mapInstance, { lat: 48.856614, lng: 2.3522219 }, 'A', 'Départ');
            addCustomMarker(mapInstance, { lat: 48.866614, lng: 2.3622219 }, 'B', 'Arrivée');
            
            const bounds = new window.google.maps.LatLngBounds();
            bounds.extend({ lat: 48.856614, lng: 2.3522219 });
            bounds.extend({ lat: 48.866614, lng: 2.3622219 });
            mapInstance.fitBounds(bounds);
          } catch (markerError) {
  ...
  // Fonction pour ajouter des marqueurs personnalisés
  const addCustomMarker = (mapInstance, position, label, title) => {
    const backgroundColor = label === 'A' ? '#d4af37' : '#1c2938'; // Couleurs primaire et secondaire d'ELC
