...
export const authOptions = {
    ...
    CredentialsProvider({
      ...
      async authorize(credentials) {
        try {
          ...
          if (!user) {
            console.log('Utilisateur non trouvé pour:', credentials.email);
            return null;
          }

          // Vérifier le statut de l'utilisateur
          if (user.status !== 'active') {
            console.log('Compte utilisateur inactif:', credentials.email);
            return null;
          }
  ...
  // Events pour debugging
  events: {
    async signIn({ user, account, profile }) {
      console.log('SignIn event:', { user: user.email, role: user.role });
    },
    async signOut({ token }) {
      console.log('SignOut event:', { user: token.email });
    },
    async createUser({ user }) {
      console.log('CreateUser event:', { user: user.email });
    },
    async session({ session, token }) {
