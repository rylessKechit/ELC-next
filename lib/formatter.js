...
export function formatDate(date) {
  ...
  export function formatPhoneNumber(phone) {
    ...
    if (cleanPhone.length === 11 && cleanPhone.startsWith('33')) {
      return `+${cleanPhone.replace(/(\d{2})(\d{2})(\d{2})(\d{2})(\d{2})(\d{2})/, '$1 $2 $3 $4 $5 $6')}`
    }
   ...
   * @param {Date} date - Date pour laquelle on veut le jour
   ...
   * @param {Date} date - Date pour laquelle on veut le mois
