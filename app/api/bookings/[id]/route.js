...
export async function GET(request, { params }) {
  try {
    ...
    // Si l'utilisateur est un chauffeur, ajouter la condition d'assignation
...
export async function PUT(request, { params }) {
  try {
    ...
    // Si l'utilisateur est un chauffeur, vérifier qu'il est assigné à cette réservation
    if (session.user.role === 'driver' && booking.assignedDriver && booking.assignedDriver.toString() !== session.user.id) {
    ...
    if (oldStatus !== booking.status && process.env.EMAIL_USER && process.env.EMAIL_PASSWORD) {
      try {
        ...
        switch(booking.status) {
          case 'confirmed':
            statusTitle = 'Réservation confirmée';
            statusMessage = 'Votre réservation a été confirmée par notre équipe. Un chauffeur vous contactera avant votre prise en charge.';
            break;
          case 'cancelled':
            statusTitle = 'Réservation annulée';
            statusMessage = 'Votre réservation a été annulée. Si vous avez des questions, n\'hésitez pas à nous contacter.';
            break;
          case 'completed':
            statusTitle = 'Course terminée';
            statusMessage = 'Votre course a été marquée comme terminée. Merci d\'avoir utilisé nos services, nous espérons vous revoir bientôt.';
            break;
          default:
            statusTitle = 'Mise à jour de votre réservation';
        ...
        const mailOptions = {
          ...
          subject: `${statusTitle} - Elysian Luxury Chauffeurs [${booking.bookingId}]`,
          html: `
            <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #eee;">
              <div style="text-align: center; margin-bottom: 20px;">
                <h1 style="color: #d4af37;">${statusTitle}</h1>
              </div>
              ...
              <div style="background-color: #f8f9fa; padding: 15px; border-radius: 5px; margin: 20px 0;">
                <h2 style="color: #1c2938; margin-top: 0;">Détails de votre course</h2>
              ...
              <p style="background-color: #fff3cd; padding: 10px; border-left: 4px solid #ffc107; margin: 15px 0;">
                Pour toute question, veuillez nous contacter au <a href="tel:+33643537653">+33 6 43 53 76 53</a>.
              </p>
              
              <div style="margin-top: 30px; padding-top: 20px; border-top: 1px solid #eee; text-align: center; color: #666; font-size: 14px;">
                <p>Merci de votre confiance,</p>
                <p><strong>L'équipe Elysian Luxury Chauffeurs</strong></p>
              </div>
            </div>
            `,
        };
